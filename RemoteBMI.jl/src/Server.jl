...
function get_value(req::HTTP.Request, name::String;)::Vector{Float64}
  ...
  values = BMI.get_value(m, name, dest)
  return values
end
...
function get_value_at_indices(
  ...
  jindices = indices .+ 1
  dest = reserve_get_value_at_indices(m, name, jindices)
  return BMI.get_value_at_indices(m, name, dest, jindices)
end
...
function get_var_type(req::HTTP.Request, name::String;)::GetVarTypeResponse
  ...
  if type == Any
    error(
      "Invalid data type returned by model: $raw_type, allowed types are: Float64, Float32, Int64, Int32",
    )
  end
