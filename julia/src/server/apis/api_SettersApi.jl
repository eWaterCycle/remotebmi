...
function set_value_read(handler)
  function set_value_read_handler(req::HTTP.Request)
    ...
    openapi_params["request_body"] =
      OpenAPI.Servers.to_param_type(Vector{Float64}, String(req.body))
...
function set_value_at_indices_read(handler)
  function set_value_at_indices_read_handler(req::HTTP.Request)
    ...
    path_params = HTTP.getparams(req)
    openapi_params["name"] = OpenAPI.Servers.to_param(String, path_params, "name"; required = true)
    openapi_params["SetValueAtIndicesRequest"] =
      OpenAPI.Servers.to_param_type(SetValueAtIndicesRequest, String(req.body))
    req.context[:openapi_params] = openapi_params
...
function registerSettersApi(
  ...
  HTTP.register!(
    router,
    "POST",
    path_prefix * "/set_value_at_indices/{name}",
    OpenAPI.Servers.middleware(
      impl,
      set_value_at_indices_read,
      set_value_at_indices_validate,
      set_value_at_indices_invoke;
      optional_middlewares...,
    ),
  )
  return router
